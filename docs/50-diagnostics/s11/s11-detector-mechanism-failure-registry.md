...
The R19 local-validation evidence supersedes any earlier registry wording that
described R19 as design-only or focused-test-only. It remains field-unqualified
until the canonical Windows replay.
...
The later R19 local-validation result supersedes the preceding design-only
wording: the bounded release-chain implementation is present and passed local
validation, while canonical Windows field proof remains pending.
