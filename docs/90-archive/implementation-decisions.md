...
This is a historical baseline decision log, not a project-status document. The [product SSOT](../rotary_oil_level_tracker_ssot_spec.md) remains authoritative, and an active feature architecture overrides an older detector implementation note where they differ. In particular, detector items 8, 11 and 12 describe the pre-cutover baseline; current S5-B authority is the [oil-boundary hypothesis architecture](../20-architecture/s5b-oil-boundary-hypothesis-architecture.md).
