...
The completed bidirectional Foam diagnostic identifies the next current-frame defect inside the S5-A Foam owner. On sample3 blind-positive Foam rows, the mandatory whiteness membership gate eliminates support before connected-component formation on `18/18`; on sample4 blind-negative representatives, bright textured wide/hollow rim/refractive structure enters support and acquires false Foam authority on `14/14`, while full replay remains Foam `113/113`. `FoamTemporalGate` is not the earliest owner in either direction, and D1 correctly withholds sample4 Oil-context authority `113/113` but intentionally does not repair S5-A publication or fill-state semantics. The next source slice is Lane C `S11-D4 Foam Support Representation / Structural Component Classification Repair`: treat absolute whiteness as evidence rather than universal membership where independent Foam morphology supports a darker/yellow component, and prevent wide/hollow structural/refractive components from obtaining S5-A publication authority. Representation and classification belong in one coherent slice because loosening support alone can increase structural false positives, while classification-only repair leaves the sample3 support loss untouched. D1 remains defense in depth and temporal trajectory work remains downstream.
