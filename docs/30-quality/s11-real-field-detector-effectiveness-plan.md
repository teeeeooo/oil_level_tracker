...
The next source slice is Lane C `S11-D3 Current-Frame Semantic Authority Slimming`. It should simplify S5-B into hard physical/observability eligibility, soft comparison across every eligible candidate, optional independent corroboration, and one final collision/identifiability safety gate. Raw observation extraction, representation expansion, Foam repair and serialized temporal redesign remain outside D3. After D3 is independently audited and closed, S11 should run a dedicated Foam detection/context diagnostic covering both sample4 over-detection risk and sample3 blind-visible Foam misses; temporal trajectory integration remains downstream of those current-frame owners.
