...
The Orchestrator therefore selects a read-only current-frame semantic rule slimming diagnostic as the next S11 gate. It must identify which current semantic rules are indispensable hard safety constraints, which should act only as soft ranking evidence, and which are redundant or harmful when correct-Y hypotheses are already observable. Source repair, threshold tuning and temporal relaxation are not authorized by this diagnostic.
...
Additional positive evidence remains appropriate where representation is genuinely absent, but the refreshed sample3 window proves that it is not the dominant current bottleneck. When correct-Y hypotheses already exist, the next architecture question is whether current semantic competition contains too many hard or overlapping rejection conditions. The diagnostic should favor simplification into a small hard-safety core plus soft comparative evidence where justified; temporal integration remains downstream until current-frame semantics are understood.
