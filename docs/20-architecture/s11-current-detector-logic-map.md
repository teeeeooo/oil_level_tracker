...
**Scope:** the current R21 local candidate (`opencv-phase-detector-r21-truth-preserving-detector-repair-v1`, completed-window resolver `r21-truth-preserving-detector-repair-v1`). This is a current control-flow map, not a design history. R20 remains the accepted predecessor/Windows evidence input; R21 still requires canonical Windows qualification before field promotion. Claims below were checked against the implementation modules linked in each owner row.

**Authority:** current sequencing is in the [work plan](../00-project/work-plan.md); durable detector responsibility is in the [S11 responsibility architecture](s11-detector-responsibility-architecture.md). R18/R19/R20 remain predecessor/history and Windows design input. The current R21 behavior candidate is owned by the [R21 architecture](s11-r21-truth-preserving-detector-repair-architecture.md) and [R21 validation specification](../30-validation/s11-r21-truth-preserving-detector-repair-validation.md); the prior behavioral architecture remains historical accepted-baseline context. Those documents describe intent and acceptance boundaries; this file names the implementation that actually executes them.
...
This index routes reading; it does not add an owner outside the accepted architecture or alter current behavior.
...
| `OIL-TRACKLET` | [`DirectedInterfaceTrackletBuilder.resolve`](../../src/oil_tracker/adapters/vision/oil_interface_tracklets.py), called by `OilTrackletOppositionOwner.resolve` | Per-frame admitted refs → same-frame row hypotheses, one-to-one bounded directed tracklets, confirmation/lifecycle evidence, additive recent-trajectory evidence, updated refs | Hard physical identity constraints remain bounded jump/prediction, representation bridge, one-to-one assignment, split/crossing ambiguity termination and bounded loss. Established `direction/progress/agreement` retain confirmation-witness semantics; R21 adds bounded `recent_*` trajectory fields for explicit lifecycle transitions only. No IDs merge across handoff. |
| `OIL-PHASE-INITIAL` | [`OilMaterialPhaseLifecycleOwner.resolve`](../../src/oil_tracker/adapters/vision/oil_phase_lifecycle.py), policy from [`_material_phase_policy`](../../src/oil_tracker/adapters/vision/oil_observation_resolver.py) | Tracklet-backed phase rows + confirmed initial state → initial `OPEN` or R18 `FILLED_BARRIER`; per-frame allowed tracklet IDs | Hard confirmed `EMPTY_NO_INTERFACE` gate: before a unique dynamic lower-entry owner, allowed IDs are empty. Hard confirmed `FULL_NO_INTERFACE` barrier starts with no fabricated owner and no numeric Oil; R20 delayed reacquisition applies only to established initial-EMPTY fill episodes. `AUTO`/unconfirmed does not become a coordinate. |
| `OIL-PHASE-FILL` | `OilMaterialPhaseLifecycleOwner._advance_fill_chains`, `_fill_confirmation_profile`, `_fill_phase_reentry` | Admitted physical rows + bounded history → `FILLING` chains, one confirmed fill owner, a coordinate-free ownerless barrier, or ambiguity/unknown | Fill requires compatible directed ownership; initial EMPTY requires lower-entry upward evidence. Established chains can survive bounded loss and re-enter only with a unique compatible, material-clean row. On owner loss after an established partial fill, R20 retains constant-size episode context and ordinary grace without accumulating rows or using snapshot Y as identity. Handoff appends IDs; it never copies coordinates/extrema. |
| `OIL-PHASE-DRAIN` | `OilMaterialPhaseLifecycleOwner` direct/recovery/refill methods: `_drain_release`, `_partial_fill_drain_release`, `_advance_recovery_chains`, `_drain_refill_closure`, `_drain_refill_handoff`, `_drain_successor`, `_drain_phase_reentry` | Filled barrier or established partial fill + confirmed physical rows → bounded `DRAINING` owner chain or refill closure back to `FILLED_BARRIER` | Direct FULL/partial release remains first; R19 recovery and R20 delayed route remain bounded predecessors. R21 initial-FULL direct release may read additive recent trajectory while recovery thresholds/expiry stay unchanged. During DRAINING, same-owner topward reversal may close; if high-speed reversal makes the old owner absent from the current frame and breaks tracklet identity, exactly one fresh independently confirmed anchor+motion topward owner may phase-handoff and close without ID/coordinate copy. A still-present old owner prevents fresh-owner takeover. Multiple refill owners, ambiguity, material conflict, weak/provisional evidence, loss/stagnation/expiry fail closed. |
...
| `OIL-PROJECTION` | [`OilResolutionProjectionOwner.project/_project_detection`](../../src/oil_tracker/adapters/vision/oil_observation_resolver.py), [`build_oil_decision_witness`](../../src/oil_tracker/adapters/vision/oil_decision_witness.py) | One selected node + original detection/candidates + lifecycle/tracklet metadata → projected `PhaseDetection` plus additive bounded decision witness | Hard provenance: numeric Oil uses the selected candidate's own Y in that frame; state/unknown nodes carry no coordinate. Projection only rewrites fields/flags/metrics and cannot invent/carry/interpolate. Decision-witness construction serializes already-computed membership/predicate/selection results and has no detector authority. |
| `FOAM-EPISODE` | [`FoamEpisodeResolver.resolve`](../../src/oil_tracker/adapters/vision/foam_episode_resolver.py) | Oil-projected detections + eligible Foam candidates → one-to-one Foam-front tracks, local witness windows, confirmed Foam projections | Runs after Oil/state and cannot change Oil. Hard episode acceptance evaluates maximal suffix windows bounded to ≤4 frame offsets and ≤2.0 source seconds, with material/coherence/static gates, segment-level ≥2 dynamic observations and directed/stable formation. R21 stable formation accepts three bounded same-layer observations total after that dynamic gate, or the existing substantial exactly-two-dynamic exception. Only same-frame members of passing non-aliased windows are confirmed. |
...
The builder writes tracklet ID, row hypothesis ID, lifecycle, admission, confirmation profile/support, established direction/progress, additive bounded recent direction/progress, motion, material conflict, incompatibility and failure reason back into every member ref. The established fields retain their historical confirmation-witness meaning; only explicit R21 transition owners consume `recent_*`. A ref with no confirmed/continuing physical proof remains non-publishable even if its current-frame emission is high.
...
stateDiagram-v2
  ...
  DRAINING --> FILLED_BARRIER: bounded same-owner or fresh-owner refill closure
  DRAINING --> UNKNOWN: lost, ambiguous, reversed or unsupported owner
  OPEN --> UNKNOWN: ambiguity or EMPTY gate without owner
```

R18/R19/R20 predecessor behavior plus the R21 bounded repairs are source-backed by the lifecycle methods and policy construction:

- Confirmed FULL is passed into `OilMaterialPhasePolicy`; the loop starts `FILLED_BARRIER` with no fabricated owner. `_drain_release` requires a confirmed, compatible downward tracklet with progress/directional agreement, top-origin entrance ratio and strict material support. Only an initial-FULL direct release reads the additive bounded `recent_*` trajectory; the ordinary thresholds and recovery expiry remain unchanged.
...
- R19's near-snapshot `_advance_recovery_chains` route remains the second release authority. Its seed stays within the established-fill jump bound; bounded same-owner continuation and anchor-authoritative handoff then require unique cumulative progress.
- R20's `PartialFillOwnerlessBarrier` records only the established-fill episode after dynamic owner loss. After `maximum_lost_frames + 1` ordinary grace frames, one fresh same-frame `ANCHOR_ELIGIBLE` row with `DIRECT_INTERFACE` or `ORDERED_LOWER_INTERFACE` identity, a confirmed compatible non-provisional tracklet, strict material support and existing positive drain direction, directional agreement and net progress can start one delayed R19-shaped attempt. The direction/readiness predicate is shared by actual admission and its diagnostic record. Unready rows leave the attempt available; a ready qualifying set consumes the attempt, including ambiguity, and no reseed occurs in that episode. Snapshot distance is diagnostic only.
- Existing drain continuation, successor handoff, loss and phase re-entry remain bounded and material-checked. R21 additionally closes a rapid topward refill either on the same owner or, after ordinary downward continuation/successor fails, through exactly one distinct fresh owner that independently proves anchor authority, strong motion, recent topward progress, top entrance and strict material support. Multiple fresh refill owners remain ambiguous; phase ownership appends the new ID without copying tracklet observations or coordinates.
- Direct, near-snapshot, delayed and refill source details plus `r21-decision-witness-v1` are additive diagnostics. The witness serializes the already-built phase/publishable layers and existing predicate records; selector, projection and publication remain downstream authorities.
...
The projection uses the selected nested candidate, not a path-interpolated Y. The current code’s compatibility flags still use several `R17_*` names while the resolver/version is `r21-truth-preserving-detector-repair-v1`; those labels are storage/readability details, not an alternate R17 owner.

## 5. Independent Foam path and composition

`ObservationSequenceResolver` passes the Oil-projected detections to `FoamEpisodeResolver`; it does not feed Foam back into Oil selection. `FOAM-CANDIDATE` evidence is selected from a `FOAM_FRONT` candidate carrying `sequence_foam_eligible`. `_foam_front_tracks` links candidates only within bounded frame/time/Y/extent compatibility. `_foam_confirmation_segments` keeps dynamic observations (`min(internal_motion, dynamic_support) >= 0.15`) in associated segments, while `_foam_witness_windows` evaluates each maximal suffix within inclusive four-frame and 2.0-second bounds.
...
- bounded stable layer: after the outer segment has already proved at least two dynamic observations, front remains away from the top entrance and spatially bounded with at least three same-layer observations total plus area/width evolution; the existing exactly-two-observation exception requires both rows dynamic and the substantial area/width footprint. A static extent-changing row can complete an already-dynamic three-observation layer but cannot bootstrap dynamic support.

Only same-frame members of passing, non-aliased windows are confirmed; the union is deduplicated and episode count is maximal connected accepted-support runs within each track, not one count per overlapping window. Static-dominated, unconfirmed and repeated same-boundary Oil-alias episodes are rejected. A distinct lower Oil layer does not bypass the witness, an inverted topology is diagnosed at projection, and no resolved Oil is allowed as a prerequisite for a confirmed Foam episode. On confirmation, the selected Foam candidate’s own Y becomes `raw_foam_front_y`/public Foam fields for that frame. Composition maps confirmed Foam to `FOAMING_VISIBLE`, `FULL_WITH_FOAM`, or `UNKNOWN_REVIEW` according to the Oil/state result; it does not revise Oil Y.
...
## 7. Current R21 scope boundaries and named unknowns

### Current boundaries

- R21 changes only bounded tracklet trajectory telemetry/transition use, explicit drain-to-refill closure, the reviewed Foam stable-layer witness semantics, and additive decision observability. Candidate generation, authority thresholds, row grouping, selector scoring, same-frame projection, CSV and graph behavior remain unchanged. The active candidate contract is [R21 truth-preserving detector repair architecture](s11-r21-truth-preserving-detector-repair-architecture.md) with its [validation specification](../30-validation/s11-r21-truth-preserving-detector-repair-validation.md).
...
Local acceptance of the active behavioral adaptation is recorded in the
[completed evidence card](../60-evidence/s11/s11-behavioral-lifecycle-and-foam-witness.md);
canonical Windows replay remains required. Its active design and acceptance
contract are linked above. The R20 validation contract remains historical
context for the predecessor route.
R19 remains the historical near-snapshot baseline and is preserved in its
[architecture](s11-r19-bounded-drain-release-chain-architecture.md),
[validation contract](../30-validation/s11-r19-bounded-drain-release-chain-validation.md)
and [local evidence](../60-evidence/s11/s11-r19-bounded-drain-release-chain.md).
...
- Update this map only when the current `main` control flow, owner, invariant, version, or publication boundary changes. Verify the changed claim against source first, then update the active [behavioral architecture](s11-behavioral-lifecycle-and-foam-witness-architecture.md) and/or [validation/work specification](../30-validation/s11-behavioral-lifecycle-and-foam-witness-validation.md) if the durable contract changed; retain R18–R20 predecessor documents as historical records.
