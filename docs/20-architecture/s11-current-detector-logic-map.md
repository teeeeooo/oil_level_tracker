...
**Scope:** the detector currently on `main` (`opencv-phase-detector-r20-delayed-drain-reacquisition-v1`, completed-window resolver `r20-delayed-drain-reacquisition-v1`). This is a current control-flow map, not a design history. Claims below were checked against the implementation modules linked in each owner row.

**Authority:** current sequencing is in the [work plan](../00-project/work-plan.md); durable detector responsibility is in the [S11 responsibility architecture](s11-detector-responsibility-architecture.md); R18 remains failed behavior history, R19/R20 preserve predecessor lifecycle routes, and the active delayed-readiness/Foam-witness behavior is owned by the [behavioral architecture](s11-behavioral-lifecycle-and-foam-witness-architecture.md) and [validation/work specification](../30-validation/s11-behavioral-lifecycle-and-foam-witness-validation.md). Those documents describe intent and acceptance boundaries; this file names the implementation that actually executes them.
...
| `OIL-PHASE-DRAIN` | `OilMaterialPhaseLifecycleOwner` direct and recovery methods: `_drain_release`, `_partial_fill_drain_release`, `_advance_recovery_chains`, `_drain_successor`, `_drain_phase_reentry` | Filled barrier or established partial fill + confirmed downward rows → `DRAINING` owner chain and allowed IDs | Direct FULL/partial release remains first. Near-snapshot recovery remains second. The delayed route adds one post-grace attempt per established-fill episode, and consumes it only for a unique fresh same-frame anchor with valid phase identity, confirmed compatible tracklet, strict material support and existing positive drain direction/agreement/progress. Ambiguous/lost/material-opposed/stagnant/expired chains fail closed; IDs stay distinct and only the current row is publishable. |
...
| `FOAM-EPISODE` | [`FoamEpisodeResolver.resolve`](../../src/oil_tracker/adapters/vision/foam_episode_resolver.py) | Oil-projected detections + eligible Foam candidates → one-to-one Foam-front tracks, local witness windows, confirmed Foam projections | Runs after Oil/state and cannot change Oil. Hard episode acceptance evaluates maximal suffix windows bounded to ≤4 frame offsets and ≤2.0 source seconds, with material/coherence/static gates and directed/stable formation. Stable support counts actual dynamic rows; only same-frame members of passing non-aliased windows are confirmed, deduplicated and counted as maximal accepted-support runs. Trace-only IDs/predicates remain diagnostics. Repeated final-Oil same-boundary alias rejects; separated Oil does not. |
...
R18/R19/R20 behavior is source-backed by the lifecycle methods and policy construction:
...
- Existing drain continuation, successor handoff, loss and phase re-entry remain bounded and material-checked. Ambiguity returns an empty allowed set and marks the frame ambiguous. Direct, near-snapshot and delayed source details are additive diagnostics; selector, projection and publication remain downstream authorities.
...
The projection uses the selected nested candidate, not a path-interpolated Y. The current code’s compatibility flags still use several `R17_*` names while the resolver/version is `r20-delayed-drain-reacquisition-v1`; those labels are storage/readability details, not an alternate R17 owner.
...
- bounded stable layer: front remains away from the top entrance and spatially bounded, with at least three dynamic observations or exactly two dynamic observations having the existing substantial area-and-width footprint, plus area/width evolution. A static extent-changing row is not dynamic support.
...
## 7. Current R20 scope boundaries and named unknowns

### Current boundaries

- The active behavioral adaptation changes only delayed Oil lifecycle admission around unchanged direct/near recovery and local Foam confirmation windows. Candidate generation, authority thresholds, directed tracklets, selector ordering, same-frame projection, CSV and graph behavior remain unchanged. The active behavior contract is [S11 behavioral lifecycle and Foam witness architecture](s11-behavioral-lifecycle-and-foam-witness-architecture.md) with its [validation/work specification](../30-validation/s11-behavioral-lifecycle-and-foam-witness-validation.md).
