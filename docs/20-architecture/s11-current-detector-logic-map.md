...
- R19's near-snapshot `BoundedOilReleaseEvidenceEngine.advance` route remains the second release authority. Its seed stays within the established-fill jump bound; bounded same-owner continuation and anchor-authoritative handoff then require unique cumulative progress.
...
## R22 ownership/evidence replacement

The current Oil phase/evidence seam is implemented by
`OilAdmissionEvidenceOwner`, `OilTrackletOppositionOwner`,
`OilMaterialPhaseLifecycleOwner`, `BoundedOilReleaseEvidenceEngine`, and
`OilResolutionProjectionOwner`. `OilFrameDecision` is the immutable per-frame
decision surface; typed `OilIdentityContradiction` survives authority and row
ownership. Initial-FULL recovery alone may earn one same-owner evidence lease
renewal before expiry, capped at twice the original window. Cached tracklet
witness start/end/count/current endpoint fields are diagnostic evidence, not
selection authority. Partial-fill and delayed contexts retain their existing
bounded expiry and one-attempt semantics.
