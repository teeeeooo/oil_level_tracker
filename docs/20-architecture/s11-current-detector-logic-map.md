...
**Authority:** current sequencing is in the [work plan](../00-project/work-plan.md); durable detector responsibility is in the [S11 responsibility architecture](s11-detector-responsibility-architecture.md); R18 remains the failed behavior authority, R19 preserves the near-snapshot route, and R20 is the current local successor under its [architecture](s11-r20-delayed-drain-reacquisition-architecture.md) and [validation contract](../30-validation/s11-r20-delayed-drain-reacquisition-validation.md). Those documents describe intent and acceptance boundaries; this file names the implementation that actually executes them.
...
| `OIL-PHASE-DRAIN` | `OilMaterialPhaseLifecycleOwner` direct and recovery methods: `_drain_release`, `_partial_fill_drain_release`, `_advance_recovery_chains`, `_drain_successor`, `_drain_phase_reentry` | Filled barrier or established partial fill + confirmed downward rows → `DRAINING` owner chain and allowed IDs | R18 direct FULL/partial release remains first. R19 near-snapshot recovery remains second. R20 adds one post-grace delayed attempt per established-fill episode, seeded only by a unique fresh same-frame anchor with valid phase identity, confirmed compatible tracklet and strict material support; existing bounded step/handoff/progress/loss rules then apply. Ambiguous/lost/material-opposed/stagnant/expired chains fail closed; IDs stay distinct and only the current row is publishable. |
...
- R20 changes only bounded Oil lifecycle admission around unchanged R18 direct paths and R19 near-snapshot recovery. R18 still owns lifecycle initial-state ownership, established partial-fill reversal and bounded Foam-front formation. R20 adds no candidate-generation, authority-threshold, directed-tracklet, selector-ordering, same-frame projection, CSV, graph or Foam behavior.
...
R20 is locally validated at source HEAD; canonical Windows replay remains
pending. Its design and acceptance contract are linked above and in the
