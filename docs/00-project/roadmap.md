...
- **Status:** `ACTIVE`
- **Major result:** S11-B and D1–D5 plus the accepted comparative-anchor repair established the current-frame representation, Spatial, Foam/Oil context and semantic-authority baseline. Fresh post-repair reconciliation found no coherent successor current-frame repair, while a distinct family remains where materially aligned current-frame Oil authority is first lost in serialized temporal/reacquisition handling; wrong current-frame selections also show that temporal continuity can reinforce bad evidence.
- **Next gate:** **Read-only intermittent-boundary / temporal-reacquisition diagnostic:** isolate why successful bounded reacquisition works in one sequence but later aligned boundaries are lost across ambiguity or movement consistency, and determine whether any safe existing-owner temporal mutation is actually justified. Production temporal mutation, final Windows field-workflow validation and S11 closure remain unapproved until that diagnostic decision.
- **Priority:** `P0`
