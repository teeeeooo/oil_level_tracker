...
- **Next gate:** Complete and independently verify detector-pipeline responsibility modularization on top of the independently verified MainWindow structural head; only then execute final target-Windows field-workflow validation on the resulting exact head.
- **Priority:** `P0`
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery and numeric trajectory-estimation responsibility, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D, S11-R1 and S11-R2 are accepted, and the coverage-first detector contract remains binding. MainWindow responsibility modularization is independently verified; the exact current S11 gate is **behavior-preserving detector-pipeline responsibility modularization and independent verification** before the deferred final target-Windows field-workflow validation. S12 remains blocked until S11 closure.
