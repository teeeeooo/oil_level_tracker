...
- **Status:** `ACTIVE`
- **Major result:** R18/R19 direct and near-snapshot lifecycle safety remains preserved. The approved behavior slice now admits delayed drain attempts only after existing positive direction/agreement/progress and bounds Foam formation authority to overlapping local four-frame/2.0-second witness windows with actual dynamic stable support. Local implementation, validation, independent audit and final Sol review are complete in the [behavioral lifecycle and Foam witness evidence](../60-evidence/s11/s11-behavioral-lifecycle-and-foam-witness.md). The user-reported Windows result remains `FIELD FAIL`; local behavior evidence is not a field qualification, and the old field comparison requirements remain unfulfilled.
- **Next gate:** Preserve the accepted local behavioral contract while any future deliberate Windows field qualification is separately authorized. This does not reopen the closed Windows investigation, authorize automatic replay on private media, or authorize a new revision. Base/Accum residuals and the optional decision-witness package remain non-current retained work.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [active behavioral architecture](../20-architecture/s11-behavioral-lifecycle-and-foam-witness-architecture.md), [active behavioral validation](../30-validation/s11-behavioral-lifecycle-and-foam-witness-validation.md), [completed behavioral evidence](../60-evidence/s11/s11-behavioral-lifecycle-and-foam-witness.md), [R18 architecture](../20-architecture/s11-r18-lifecycle-closure-architecture.md), [R19 architecture](../20-architecture/s11-r19-bounded-drain-release-chain-architecture.md), [R20 predecessor architecture](../20-architecture/s11-r20-delayed-drain-reacquisition-architecture.md), [R20 Windows evidence consolidation](../60-evidence/s11/s11-r20-windows-evidence-consolidation.md), [R20 decision-witness architecture](../20-architecture/s11-r20-decision-witness-observability-architecture.md), [R20 decision-witness validation/work specification](../30-validation/s11-r20-decision-witness-observability-validation.md), [canonical reviewed truth](../30-validation/windows-sample1-heating-coldstart-reviewed-truth.md)
...
`S6 DONE` → `S7 / Phase 2C-4 DONE` → `S8 DONE` → `S9 DONE` → `S10 DONE` → `S11 ACTIVE (R17/R18 Windows FAIL → R20 report FIELD FAIL → approved behavioral implementation/local acceptance complete; Windows qualification remains required)` → `S11-M/S12 implementation slices PLANNED`.

Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, unresolved Base/Accum transition designs and optional decision-witness observability, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R18 failed private field holdouts or operator checks. R19/R20 predecessor behavior remains historical context, while the active behavioral implementation and local acceptance are recorded in the [completed evidence card](../60-evidence/s11/s11-behavioral-lifecycle-and-foam-witness.md). The old field qualification/comparison requirements remain unfulfilled, not waived or executed; no automatic private replay or reopened Windows investigation is current, S11 remains active, and no field-qualified detector is claimed. The unmerged `codex/ui-ux-refresh` work is outside the current detector repair and is not part of `main`.
