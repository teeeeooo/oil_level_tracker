...
- **Status:** `VALIDATING`
- **Major result:** The private R7 holdout failed with Base 0/601 and Accum 37/601 numeric Oil. R8 is now the implemented generic authority: it separates Foam texture from Oil, adds bounded high-recall proposals and continuation, preserves independent confirmed Foam, supports user-selected normalized artifact templates, labels all-missing confirmed-initial-state graph holds and reduces measured local detector time. Local four-video and user-like calibration replays passed.
- **Next gate:** Replay the exact pushed R8 head on secure-Windows Base/Accum with calibration off and with operator-selected detector proposals, then reconcile Oil/Foam graphs, events, captures and debug-disabled frame time against source frames.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [R8 architecture](../20-architecture/s11-r8-observation-recovery-architecture.md), [R8 validation](../30-validation/s11-r8-observation-recovery-validation.md), [R8 local evidence](../60-evidence/s11/s11-r8-observation-recovery.md), [R7 Windows root cause](../50-diagnostics/s11/s11-r7-windows-observation-recovery-diagnostic.md), [durable S11 detector architecture](../20-architecture/s11-detector-responsibility-architecture.md), [Windows field-workflow checklist](../40-operations/manual-gui-windows-checklist.md)
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R7 failed private field holdouts. R8 is the implemented authority under private-Windows validation. S11-M remains blocked until the private-Windows and packaged-workflow S11 gate closes, and S12 follows S11-M.
