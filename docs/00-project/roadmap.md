...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery; the P2 exposure-decoupled no-interface repair, its genuine FULL/EMPTY preservation correction and the conservative Spatial positive-evidence fallback are accepted production behavior. The Offline Temporal Trajectory probe did not justify result-layer interpolation. The local-corpus canonical portability repair is also merged: absent ignored MP4s are explicit corpus skips, wrong identities fail closed, and historical P0 diagnostics are separated from current Spatial production. No detector/general-field accuracy PASS or S11 completion is inferred.
- **Next gate:** Rerun the full Windows canonical suite on the portability-repaired main; return to controlled Windows field re-validation only if canonical is green.
- **Priority:** `P0`
