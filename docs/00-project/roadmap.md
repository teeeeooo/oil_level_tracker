...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery; the P2 exposure-decoupled no-interface repair and conservative Spatial positive-evidence fallback are now accepted production behavior. The subsequent Offline Temporal Trajectory probe did not justify result-layer interpolation: held-out accepted-anchor reconstruction recovered none of the four residual production misses and showed material errors when stable accepted observations followed a persistent wrong boundary. Production trajectory integration is therefore not selected from current evidence. No detector/general-field accuracy PASS or S11 completion is inferred.
- **Next gate:** Re-run the original Windows field workload with the accepted P2 + Spatial production detector and compare it against the recorded pre-S11 Base/Accum field behavior, with representative visual review to distinguish genuine Oil recovery from new false numeric boundaries.
- **Priority:** `P0`
- **Detail:** [S11 real-field detector effectiveness plan](../30-quality/s11-real-field-detector-effectiveness-plan.md), [S11-A detector direction and experiment plan](../30-quality/s11-a-detector-direction-and-experiment-plan.md), [S11-B Spatial production fallback](../30-quality/s11-b-spatial-positive-evidence-production.md), [S11 Offline Temporal Trajectory probe](../30-quality/s11-offline-temporal-trajectory-probe.md)
