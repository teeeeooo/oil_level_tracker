...
- **Status:** `ACTIVE`
- **Major result:** S11-A/B repairs, S11-C forensics, S11-D1, both bounded S11-D2 recovery classes, and S11-D3 current-frame semantic authority slimming are independently accepted and merged. The completed bidirectional Foam diagnostic now identifies one shared S5-A representation/classification defect: sample3 blind-positive Foam is lost at the mandatory whiteness support gate on `18/18` rows, while sample4 blind-negative structural/refractive evidence enters Foam support and acquires false authority on `14/14` representatives; full sample4 replay still publishes Foam `113/113`. `FoamTemporalGate` is downstream of both failures, and D1 correctly withholds sample4 Oil-context authority without correcting the wrong S5-A Foam publication/fill-state semantics.
- **Next gate:** Execute Lane C **S11-D4 Foam Support Representation / Structural Component Classification Repair** in the S5-A current-frame Foam owner, jointly recovering darker/yellow genuine Foam support and preventing wide/hollow structural/refractive components from obtaining Foam publication authority. Preserve D1 as Oil-routing defense in depth; temporal redesign remains downstream.
- **Priority:** `P0`
