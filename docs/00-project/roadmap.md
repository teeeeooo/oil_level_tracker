...
- **Status:** `ACTIVE`
- **Major result:** The accepted D1–D5/current-frame, serialized temporal, retrospective, Workbench interaction and observed-anchor presentation repairs remain the baseline. Subtractive Detector Simplification Slice A — Proposal Authority Consolidation is accepted and merged, removing redundant distributed-Sobel supplemental proposal/semantic authority while preserving required current behavior and canonical publication safety.
- **Next gate:** Execute **S11 Slice B — Scoped Plateau / Collision Authority** under the coverage-first acceptance contract: materially increase usable numeric trajectory coverage / reduce ambiguity gaps while keeping gross wrong-interface tracking bounded. Final Windows field-workflow validation resumes only after the simplified detector source sequence is reconciled and accepted.
- **Priority:** `P0`
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery and the evidence-gated trajectory provenance responsibility, is routed through [`retained-commitments.md`](retained-commitments.md). Slice A is accepted and merged, and the coverage-first product/architecture/validation reconciliation is complete. The exact current S11 gate is **S11 Slice B — Scoped Plateau / Collision Authority**; final Windows field-workflow validation remains the later closure gate after detector-source reconciliation.
