...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery and selected separate P2, Spatial and temporal responsibilities. The P2 exposure-decoupled no-interface production repair is accepted and merged. A focused Spatial production fallback is now implemented on top of exact main `1e83aac0643b0734fa1d67dfa30b88da6f3bd31e`: it is P0/D4-first, requires genuinely cross-ROI current-frame phase evidence before publishing a relative-phase candidate, reproduces native Oil recovery `7/13 → 9/13` without changing the original seven numeric anchors, and adds no temporal owner/state, setting, schema or dependency. The feature remains unaccepted until fresh exact-head audit. Temporal trajectory estimation remains a separate result-layer responsibility.
- **Next gate:** Fresh Lane C Independent Auditor exact-head review of the Spatial production fallback; if accepted and merged, proceed to the separately owned offline temporal trajectory probe. Valid flat/near-horizontal Oil remains a possible ambiguity and is not physically rejected by the Spatial gate.
- **Priority:** `P0`
- **Detail:** [S11 real-field detector effectiveness plan](../30-quality/s11-real-field-detector-effectiveness-plan.md), [S11-A detector direction and experiment plan](../30-quality/s11-a-detector-direction-and-experiment-plan.md), [S11-B Spatial production fallback](../30-quality/s11-b-spatial-positive-evidence-production.md)
