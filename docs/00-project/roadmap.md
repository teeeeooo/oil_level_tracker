...
- **Status:** `ACTIVE`
- **Major result:** S11-A/B repairs, S11-C forensics, S11-D1 and both bounded S11-D2 recovery classes are independently accepted and merged. The post-D2 blind refresh found visible Oil on `22/23` sample3 production-schedule rows, with material current-frame representation on all `22/22` and in-range representation on `19/22`, yet production published Oil `0/23`. The completed semantic slimming diagnostic attributes `16/17` semantic-loss rows to rejection and `1/17` to wrong selection: correct-Y evidence is repeatedly suppressed by rank-1 monopoly, subthreshold no-interface competition and overlapping boundary/artifact/ambiguity/morphology vetoes. It confirms a source change is justified and that genuine collision, affirmative no-interface, authoritative Foam topology and invalid visibility/geometry can remain the hard-safety core.
- **Next gate:** Execute Lane C `S11-D3 Current-Frame Semantic Authority Slimming`: separate the S5-B hard-safety core from soft comparative evidence, remove duplicated veto composition, and allow all hard-safe represented Oil hypotheses to compete for current-frame authority. After D3 is independently audited and closed, run the dedicated Foam detection/context diagnostic before temporal trajectory work.
- **Priority:** `P0`
- **Detail:** [S11 real-field detector effectiveness plan](../30-quality/s11-real-field-detector-effectiveness-plan.md), [S11-C full-video forensic diagnostic](../30-quality/s11-c-full-video-production-replay-visual-forensic-diagnostic.md), [S11-D1 Foam/Oil context repair](../30-quality/s11-d1-foam-structural-refractive-discrimination-oil-context-safety.md), [S11-D2 sample3 positive-evidence recovery](../30-quality/s11-d2-sample3-positive-evidence-recovery.md), [S11-D2 Class-B representation recovery](../30-quality/s11-d2-class-b-observation-proposal-representation.md), [S11-A detector direction and experiment plan](../30-quality/s11-a-detector-direction-and-experiment-plan.md), [S11-B Spatial production fallback](../30-quality/s11-b-spatial-positive-evidence-production.md), [S11 Offline Temporal Trajectory probe](../30-quality/s11-offline-temporal-trajectory-probe.md)
