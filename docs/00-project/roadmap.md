...
- **Status:** `ACTIVE`
- **Major result:** The accepted D1–D5/current-frame, serialized temporal, retrospective, Workbench interaction and observed-anchor presentation repairs remain the baseline. Subtractive Detector Simplification Slices A and B are accepted and merged: redundant distributed-Sobel supplemental authority is removed, and bright-plateau opposition is now scoped to existing collision/glare evidence while materially improving usable observed trajectory without weakening canonical publication safety.
- **Next gate:** Execute **S11 Slice C — Unified Spatial Corroboration** within the existing bounded current-frame Spatial responsibility. Final Windows field-workflow validation resumes only after the simplified detector source sequence is reconciled and accepted.
- **Priority:** `P0`
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery and the evidence-gated trajectory provenance responsibility, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A and B are accepted and merged, and the coverage-first product/architecture/validation contract remains binding. The exact current S11 gate is **S11 Slice C — Unified Spatial Corroboration**; final Windows field-workflow validation remains the later closure gate after detector-source reconciliation.
