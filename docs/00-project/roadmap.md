...
- **Status:** `ACTIVE`
- **Major result:** `S9-A — Unsaved Profile Change Tracking & Close Confirmation` is implementation-complete on its focused Worker branch and awaiting independent audit. Profile persistence dirtiness is tracked independently from `WorkbenchState` by comparing the current serialized Recipe with a safe load/save/result-snapshot baseline; current-test/session-only edits therefore do not trigger Profile-loss warnings. Application close now guards unsaved Profile changes before reader teardown with Save/Discard/Cancel while reusing the established Profile save authority. Autosave/abnormal-exit recovery, analysis-area zoom/pan/fit and field↔overlay highlight remain unstarted selection candidates; P3 remains lower-priority backlog.
