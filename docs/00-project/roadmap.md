...
- **Next gate:** Complete **Orchestrator coverage-first detector acceptance reconciliation**, then proceed to **S11 Slice B — Scoped Plateau / Collision Authority**. Final Windows field-workflow validation resumes only after the simplified detector source sequence is reconciled and accepted.
- **Priority:** `P0`
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery and the evidence-gated trajectory provenance responsibility, is routed through [`retained-commitments.md`](retained-commitments.md). Slice A is accepted and merged. The exact current S11 gate is **Orchestrator coverage-first detector acceptance reconciliation**, followed by **S11 Slice B — Scoped Plateau / Collision Authority**; final Windows field-workflow validation remains the later closure gate after detector-source reconciliation.
