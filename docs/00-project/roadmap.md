...
- **Status:** `ACTIVE`
- **Major result:** R18 implements explicit confirmed-state phase ownership, established partial-fill reversal and bounded Foam-front formation without resolver reordering or field-specific policy, but remains Windows field FAIL. The completed causal rerun freezes Base earliest-loss, Accum release, owner-bounded selector abstain and reviewed-Y unknowns, closes the Foam gates, and proves compared-field same-frame invariance without changing output. R19 near-snapshot recovery remains preserved, and R20 adds a coordinate-free ownerless partial-fill barrier plus one post-grace fresh-anchor attempt while preserving direct release, selector/projection/CSV provenance and Foam behavior. R20 local validation and four-video evidence remain separate `LOCAL PASS / WINDOWS REQUIRED` historical records; the user-reported R20 Windows field report is received and closed as `FIELD FAIL`. The old field qualification/comparison requirements remain unfulfilled, not waived or executed; Foam remains outside its scope.
- **Next gate:** Complete the documentation-only D0–D4 closeout: independent Luna routing recheck, fresh Sol final approval and the authorized scoped documentation commit. This does not mark S11 or R20 field-qualified, does not reopen the closed Windows investigation, and does not authorize automatic replay, behavior change or R21. Any deliberate future field qualification remains non-current and separately authorized through retained commitments.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [R18 architecture](../20-architecture/s11-r18-lifecycle-closure-architecture.md), [R18 validation](../30-validation/s11-r18-lifecycle-closure-validation.md), [R18 causal closure](../50-diagnostics/s11/s11-r18-windows-causal-closure.md), [R19 architecture](../20-architecture/s11-r19-bounded-drain-release-chain-architecture.md), [R19 validation](../30-validation/s11-r19-bounded-drain-release-chain-validation.md), [R19 local evidence](../60-evidence/s11/s11-r19-bounded-drain-release-chain.md), [R20 architecture](../20-architecture/s11-r20-delayed-drain-reacquisition-architecture.md), [R20 validation](../30-validation/s11-r20-delayed-drain-reacquisition-validation.md), [R20 local evidence](../60-evidence/s11/s11-r20-delayed-drain-reacquisition.md), [R20 Windows evidence consolidation](../60-evidence/s11/s11-r20-windows-evidence-consolidation.md), [R20 decision-witness architecture](../20-architecture/s11-r20-decision-witness-observability-architecture.md), [R20 decision-witness validation/work specification](../30-validation/s11-r20-decision-witness-observability-validation.md), [R18 local evidence](../60-evidence/s11/s11-r18-lifecycle-closure.md), [R18 field result](../60-evidence/s11/s11-r18-secure-windows-field-result.md), [R17 field result](../60-evidence/s11/s11-r17-secure-windows-field-result.md)
...
`S6 DONE` → `S7 / Phase 2C-4 DONE` → `S8 DONE` → `S9 DONE` → `S10 DONE` → `S11 ACTIVE (R17 Windows FAIL → R18 Windows FAIL → causal rerun complete → R20 user-reported Windows FIELD FAIL received/closed; docs-only D0–D4 closeout pending)` → `S11-M/S12 implementation slices PLANNED`.

Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R18 failed private field holdouts or operator checks. R19 remains the historical near-snapshot baseline, while R20's local implementation and full local validation remain separate historical records and its user-reported Windows field report is received/closed as `FIELD FAIL`. The old field qualification/comparison requirements remain unfulfilled, not waived or executed. The current docs-only D0–D4 closeout is pending review/final approval/scoped commit; no automatic replay or reopened Windows investigation is current, S11 remains active, and no field-qualified detector is claimed. The unmerged `codex/ui-ux-refresh` work is outside the current detector repair and is not part of `main`.
