...
- **Status:** `ACTIVE`
- **Major result:** R18 implements explicit confirmed-state phase ownership, established partial-fill reversal and bounded Foam-front formation without resolver reordering or field-specific policy, but remains Windows field FAIL. The completed causal rerun freezes Base earliest-loss, Accum release, owner-bounded selector abstain and reviewed-Y unknowns, closes the Foam gates, and proves compared-field same-frame invariance without changing output. R19 near-snapshot recovery remains preserved, and R20 adds a coordinate-free ownerless partial-fill barrier plus one post-grace fresh-anchor attempt while preserving direct release, selector/projection/CSV provenance and Foam behavior. R20 passed focused lifecycle and integration tests, the exact four-video local replay with preserved fingerprints, provenance, controlled performance, governance, compile and full local repository gates; Foam remains outside its scope.
- **Next gate:** Run the canonical Windows replay for the implemented R20 delayed drain reacquisition. R20 is locally accepted as `LOCAL PASS / WINDOWS REQUIRED`; no field-qualified detector is claimed.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [R18 architecture](../20-architecture/s11-r18-lifecycle-closure-architecture.md), [R18 validation](../30-validation/s11-r18-lifecycle-closure-validation.md), [R18 causal closure](../50-diagnostics/s11/s11-r18-windows-causal-closure.md), [R19 architecture](../20-architecture/s11-r19-bounded-drain-release-chain-architecture.md), [R19 validation](../30-validation/s11-r19-bounded-drain-release-chain-validation.md), [R19 local evidence](../60-evidence/s11/s11-r19-bounded-drain-release-chain.md), [R20 architecture](../20-architecture/s11-r20-delayed-drain-reacquisition-architecture.md), [R20 validation](../30-validation/s11-r20-delayed-drain-reacquisition-validation.md), [R20 local evidence](../60-evidence/s11/s11-r20-delayed-drain-reacquisition.md), [R18 local evidence](../60-evidence/s11/s11-r18-lifecycle-closure.md), [R18 field result](../60-evidence/s11/s11-r18-secure-windows-field-result.md), [R17 field result](../60-evidence/s11/s11-r17-secure-windows-field-result.md)
...
`S6 DONE` → `S7 / Phase 2C-4 DONE` → `S8 DONE` → `S9 DONE` → `S10 DONE` → `S11 ACTIVE (R17 Windows FAIL → R18 Windows FAIL → causal rerun complete → R20 LOCAL PASS / WINDOWS REQUIRED, Windows replay pending)` → `S11-M/S12 implementation slices PLANNED`.

Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R18 failed private field holdouts or operator checks. R19 remains the historical near-snapshot baseline, while R20 is the implementation-current local baseline with full local validation accepted; canonical Windows replay remains pending, S11 remains active, and no field-qualified detector is claimed. The unmerged `codex/ui-ux-refresh` work is outside the current detector repair and is not part of `main`.
