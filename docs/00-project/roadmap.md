...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery; the P2 exposure-decoupled no-interface repair, its genuine FULL/EMPTY preservation correction and the conservative Spatial positive-evidence fallback are accepted production behavior. The Offline Temporal Trajectory probe did not justify result-layer interpolation. The preservation repair restores genuine/noisy-uniform no-interface without reintroducing raw-brightness absence scoring; no detector/general-field accuracy PASS or S11 completion is inferred.
- **Next gate:** Repair the separate local-corpus canonical portability defect as Lane B, rerun Windows canonical on the repaired baseline, then return to controlled Windows field re-validation only after canonical is green.
- **Priority:** `P0`
