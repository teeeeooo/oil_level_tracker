...
- **Status:** `VALIDATING`
- **Major result:** The private R8 holdout failed with Base 0/601 numeric Oil and Accum Oil/Foam aliasing. R9 is now the implemented generic authority: it adds responsive bulk artifact editing, calibration-only high-recall proposals, a no-qualified-path dynamic bootstrap, current-evidence Foam alias continuation and exact final sequence diagnostics. Local uncalibrated and user-like calibration replays plus the full regression passed.
- **Next gate:** Replay the exact pushed R9 head on secure-Windows Base/Accum, review and apply the appropriate detector artifact proposals, then reconcile source-frame Oil/Foam layers, final sequence trace, graphs/events/captures and debug-disabled frame time.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [R9 architecture](../20-architecture/s11-r9-calibrated-observation-architecture.md), [R9 validation](../30-validation/s11-r9-calibrated-observation-validation.md), [R9 local evidence](../60-evidence/s11/s11-r9-calibrated-observation.md), [R8 Windows root cause](../50-diagnostics/s11/s11-r8-windows-calibrated-observation-diagnostic.md), [durable S11 detector architecture](../20-architecture/s11-detector-responsibility-architecture.md), [Windows field-workflow checklist](../40-operations/manual-gui-windows-checklist.md)
...
Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R8 failed private field holdouts. R9 is the implemented authority under private-Windows validation. S11-M remains blocked until the private-Windows and packaged-workflow S11 gate closes, and S12 follows S11-M.
