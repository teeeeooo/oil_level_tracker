...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery and selected separate P2, Spatial and temporal responsibilities. The P2 exposure-decoupled no-interface repair and the conservative Spatial positive-evidence fallback are now accepted production behavior. Spatial remains P0/D4-first, requires genuinely cross-ROI current-frame evidence before relative-phase recovery can publish, preserves valid flat/near-horizontal Oil as possible ambiguity, and adds no temporal owner/state, setting, schema or dependency. No detector/general-field accuracy PASS or S11 completion is inferred.
- **Next gate:** Run the separately owned Offline Temporal Trajectory probe on the stabilized single-frame observation stream, preserving explicit `observed` / `estimated` / `unavailable` provenance and abstention across unsupported gaps.
- **Priority:** `P0`
