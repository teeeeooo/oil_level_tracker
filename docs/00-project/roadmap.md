...
- **Status:** `ACTIVE`
- **Major result:** S11-A/B repairs, S11-C forensics, S11-D1 and both bounded S11-D2 recovery classes are independently accepted and merged. A post-D2 blind refresh of sample3 `28–40 s` then found visually clear/probable Oil on `22/23` production-schedule rows: all `22/22` retain a material current-frame representation and `19/22` are already in the blind visual Y range, yet `17/22` fail in current-observation semantic rejection/selection while only `2/22` are temporal-only suppressions. Production still publishes Oil `0/23`; visible Foam is also missed on all `18/18` blind-positive rows. The Orchestrator therefore identifies current-frame semantic identifiability/competition, not candidate absence or temporal relaxation, as the next primary S11 question.
- **Next gate:** Run a read-only S11 current-frame semantic rule slimming diagnostic that classifies which semantic rules must remain hard safety, which should become soft evidence, and which are redundant or harmful before any source repair is authorized.
- **Priority:** `P0`
