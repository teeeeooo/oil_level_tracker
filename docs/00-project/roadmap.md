...
- **Status:** `ACTIVE`
- **Major result:** R18 implements explicit confirmed-state phase ownership, established partial-fill reversal and bounded Foam-front formation without resolver reordering or field-specific policy, but remains Windows field FAIL. The completed causal rerun freezes Base earliest-loss, Accum release, owner-bounded selector abstain and reviewed-Y unknowns, closes the Foam gates, and proves compared-field same-frame invariance without changing output. R19 bounded drain-release-chain design is approved; it is not implemented or field-proven, and Foam is intentionally outside its scope.
- **Next gate:** Implement and locally validate the approved R19 bounded drain-release chain, then run the canonical Windows replay. R18 remains the implementation-current baseline and no field-qualified detector is claimed.
- **Priority:** `P0`
- **Detail:** [current work plan](work-plan.md), [R18 architecture](../20-architecture/s11-r18-lifecycle-closure-architecture.md), [R18 validation](../30-validation/s11-r18-lifecycle-closure-validation.md), [R18 causal closure](../50-diagnostics/s11/s11-r18-windows-causal-closure.md), [R19 architecture](../20-architecture/s11-r19-bounded-drain-release-chain-architecture.md), [R19 validation](../30-validation/s11-r19-bounded-drain-release-chain-validation.md), [R18 local evidence](../60-evidence/s11/s11-r18-lifecycle-closure.md), [R18 field result](../60-evidence/s11/s11-r18-secure-windows-field-result.md), [R17 field result](../60-evidence/s11/s11-r17-secure-windows-field-result.md)
...
`S6 DONE` → `S7 / Phase 2C-4 DONE` → `S8 DONE` → `S9 DONE` → `S10 DONE` → `S11 ACTIVE (R17 Windows FAIL → R18 Windows FAIL → causal rerun complete → R19 design approved, implementation/local validation next)` → `S11-M/S12 implementation slices PLANNED`.

Affirmatively retained but non-current work, including autosave/abnormal-exit recovery, is routed through [`retained-commitments.md`](retained-commitments.md). Slices A–D and S11-R1–R4 remain historical baselines; R5–R18 failed private field holdouts or operator checks. R18 remains the implementation-current `main` baseline while R19 is approved design-only work awaiting implementation/local validation and canonical Windows replay; S11 remains active and has no field-qualified detector. The unmerged `codex/ui-ux-refresh` work is outside the current detector repair and is not part of `main`.
