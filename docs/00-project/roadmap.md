...
- **Status:** `ACTIVE`
- **Major result:** S11-A probing rejected unsafe scalar relative-phase recovery and selected separate P2, Spatial and temporal responsibilities. The P2 exposure-decoupled no-interface production repair is now accepted and merged: absolute raw brightness no longer adds positive interface-absence evidence, while typed FULL/EMPTY appearance, fail-closed ambiguity and retained S5-A/S5-B protections remain intact. Spatial remains a separate positive-evidence candidate, and temporal trajectory estimation remains a separate result-layer responsibility.
- **Next gate:** Evaluate a separate conservative Spatial production fallback, preserving P0-first acceptance and valid flat/near-horizontal Oil surfaces; after the single-frame production observation stream is stabilized, probe offline temporal trajectory estimation.
- **Priority:** `P0`
