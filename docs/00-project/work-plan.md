...
Run the read-only S11 current-frame semantic rule slimming diagnostic against the refreshed sample3 `28–40 s` evidence and the canonical S5-B current-observation owner. Quantify which semantic rules reject or demote correct-Y hypotheses, distinguish indispensable hard safety from soft ranking evidence and unnecessary overlap, and recommend the smallest coherent architecture simplification before source mutation. Preserve accepted D1/D2, Spatial, genuine no-interface and serialized-temporal safety while diagnosing; do not tune thresholds or modify source in this gate.
