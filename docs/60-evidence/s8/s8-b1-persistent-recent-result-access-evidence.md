...
`RecentResultHistory` owns schema-versioned recent-result metadata in the existing application [`user_data_dir()`](../../90-archive/implementation-decisions.md) boundary. The default registry is `recent_results.json`, with a default maximum of eight entries. Official result bundles are never used as history storage and are never mutated by registration, ordering, deduplication or stale-entry handling.
