# S5-B Oil Boundary Hypothesis Architecture — Historical Development Record

> Historical evidence record. The material below preserves the prior mixed architecture/performance chronology for provenance. It no longer owns current sequencing or the durable S11 architecture. See [`../../20-architecture/s5b-oil-boundary-hypothesis-architecture.md`](../../20-architecture/s5b-oil-boundary-hypothesis-architecture.md), [`../../20-architecture/s11-detector-responsibility-architecture.md`](../../20-architecture/s11-detector-responsibility-architecture.md) and [`../../00-project/work-plan.md`](../../00-project/work-plan.md).
...
The FULL/EMPTY preservation correction keeps that exposure-decoupling intact. Removing the former `0.17 × FULL/EMPTY appearance` term reduced the positive-evidence coefficient sum from `1.00` to `0.83` while the no-interface acceptance threshold and glare/competing-boundary penalties stayed unchanged. Production assigns that removed positive weight to the already accepted exposure-decoupled normalized-raster uniformity channel, changing its coefficient from `0.27` to `0.44` while retaining weak-boundary `0.36` and visibility `0.20` unchanged. This concentrates the preservation correction on genuinely uniform scenes instead of globally lifting weak or merely visible absence evidence. Raw intensity, `full_likelihood` and `empty_likelihood` remain outside positive absence acceptance and are used only after accepted no-interface to distinguish the typed FULL versus EMPTY appearance. This restores genuinely uniform FULL/EMPTY scenes without lowering the canonical threshold, manufacturing numeric Oil, or changing temporal, projection, Spatial, Foam, schema, setting, detector-version or dependency ownership.
