...
This document owns the S11 real-field detector diagnosis, evidence plan and acceptance boundaries. Milestone state and current next action remain owned by the [roadmap](../../00-project/roadmap.md) and [work plan](../../00-project/work-plan.md). The agreed S11-A reproducible-corpus and detector-direction decision is recorded in [S11-A Detector Direction and Experiment Plan](../../50-diagnostics/s11/s11-a-detector-direction-and-experiment-plan.md). The completed forensic diagnostic contract is [S11-C Full-Video Production Replay and Visual Forensic Attribution](../../50-diagnostics/s11/s11-c-full-video-production-replay-visual-forensic-diagnostic.md). The accepted D1 repair record is [S11-D1 Foam Structural/Refractive Discrimination and Oil Context Safety](s11-d1-foam-structural-refractive-discrimination-oil-context-safety.md). The accepted D2 Class-A repair record and remaining Class-B split are [S11-D2 Sample3 Positive-Evidence Recovery](s11-d2-sample3-positive-evidence-recovery.md). The active Class-B source gate is [S11-D2 Class-B Observation/Proposal Representation Recovery](s11-d2-class-b-observation-proposal-representation.md).
...
The detailed experiment and design decision is owned by [S11-A Detector Direction and Experiment Plan](../../50-diagnostics/s11/s11-a-detector-direction-and-experiment-plan.md). The source task begins with a provisional Lane C assumption because the leading direction may change no-interface/positive-evidence semantics; it may narrow to Lane B only if the probe proves a materially smaller existing-owner defect with the S5-B contract unchanged.
...
## S11-C completed attribution and accepted S11-D1/D2 repairs
...
Fresh Lane C audits accepted D1 and both bounded D2 repair classes. D2 Class-A current-frame Spatial corroboration retains sample3 `899 → 320` and frozen `900 → 319`, the Foam-front constraint, sample2 `30 → 599` / `60 → 598`, and sample4 fail-closed Oil `0/113` / Foam `113/113`. D2 Class-B now adds a material observable representation near source Y `327` at production frame `2697` without forcing canonical or temporal promotion. Its repaired semantic admission gives ordinary hypotheses first ownership of the hard cap and protects the ordinary three-hypothesis ambiguous decision prefix; the six previously observed supplemental replacements are closed. Independent four-video comparison preserves all 444 publication, decision/tracker and serialized temporal-state/beam rows while allowing only additive supplemental candidates. The ignored S11-C forensic bundle and recorded fingerprint remain temporary non-Git evidence, not a golden fixture.

The post-D2 blind refresh materially changes the successor diagnosis. In sample3 `28–40 s`, `22/23` production-schedule rows are blindly judged to contain clear/probable Oil, all `22/22` retain material current-frame representation, and `19/22` are already in the blind visual Y range. Earliest-loss attribution is dominated by current-observation semantic rejection/selection (`17/22`), with only `2/22` temporal-only suppressions and no visually positive row lacking material representation altogether. Production nevertheless publishes Oil `0/23`. Visible Foam is independently missed on `18/18` blind-positive rows, and the provisional no-interface interpretation near `36.7 s` requires later evidence-authority reconciliation.

The completed current-frame semantic slimming diagnostic confirms the bottleneck is semantic authority rather than missing representation. Across the 17 semantic-loss rows, `16` are current-frame rejection and `1` is wrong selection; correct-Y hypotheses are frequently present but lose through rank-1-only authority and repeated, correlated boundary/artifact/no-interface/morphology vetoes. The diagnostic preserves hard safety for genuine single-frame collisions, affirmative no-interface, authoritative Foam topology and invalid visibility/geometry, while finding that boundary floors, subthreshold no-interface likelihood, coverage, paired-edge/pulse and related morphology are better treated as comparative evidence than independent vetoes. Source change is therefore justified.
...
Lane C `S11-D3 Current-Frame Semantic Authority Slimming` is **accepted and merged** through PR #90; see [S11-D3 Current-Frame Semantic Authority Slimming](s11-d3-current-frame-semantic-authority-slimming.md). Exact-head audit confirmed that affirmative no-interface remains authoritative, lower no-interface evidence is comparative rather than a repeated veto, hard observability/Foam topology safety is retained, and candidate-wise Spatial fairness remains bounded to at most `1250` sector-row evaluations under the existing hypothesis cap. Auditor execution returned `153 passed`, the fresh 17-row sample3 probe reproduced five current-frame boundaries with all five within `6 px` of the blind Oil range, and `914` plus `1049` remained fail-closed ambiguity. Raw observation extraction, representation expansion and serialized temporal redesign remain outside D3.

Lane C `S11-D4 Foam Support Representation / Structural Component Classification Repair` is **accepted and merged** through PR #91 as `9a69306146b7f10c8ece99d3dd5a16101dee875d`; see [S11-D4 Foam Support Representation / Structural Component Classification](s11-d4-foam-support-structural-classification.md). The fresh exact-head audit confirmed the simplified S5-A contract recovers sample3 blind-positive Foam `12/18`, rejects sample4 frozen structural negatives `0/14` and dense `0–32.5 s` structural negatives `0/66`, and separates later genuine Foam `14/26`. D1 remains independent defense in depth, fill-state remains downstream, and `FoamTemporalGate` plus S5-B Oil ownership are unchanged. The next gate is a read-only post-D4 S11 effectiveness reconciliation: remeasure remaining real/frozen blind losses on the accepted D3+D4 baseline before authorizing another source slice. Temporal redesign remains unselected unless refreshed application evidence demonstrates a genuinely temporal residual rather than current-frame observation or semantic loss.
