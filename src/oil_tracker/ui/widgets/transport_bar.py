...
from PySide6.QtWidgets import QComboBox, QHBoxLayout, QLabel, QPushButton, QSlider, QWidget, QStyleOptionSlider, QStyle
...
class MarkerSlider(QSlider):
    ...
    def set_markers(self, duration: float, analysis_start: float | None, analysis_end: float | None, compressor_start: float | None) -> None:
        ...
        self._markers = [(analysis_start, QColor(40, 170, 255)), (analysis_end, QColor(40, 170, 255)), (compressor_start, QColor(255, 120, 30))]
        self.update()
    ...
    def paintEvent(self, event) -> None:
        ...
        for timestamp, color in self._markers:
            ...
            painter.setBrush(color); painter.setPen(color)
            painter.drawPolygon(QPolygonF([QPointF(x, groove.top()-1), QPointF(x-5, groove.top()-8), QPointF(x+5, groove.top()-8)]))
...
class TransportBar(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.prev = QPushButton("◀ Frame")
        self.next = QPushButton("Frame ▶")
        self.slider = MarkerSlider()
        self.slider.setRange(0, 10000)
        self.time = QLabel("00:00.000 / 00:00.000 | frame 0")
        self.speed = QComboBox()
        for value in (0.5, 1.0, 1.5, 2.0, 4.0):
            self.speed.addItem(f"{value:g}×", value)
        self.speed.setCurrentIndex(1)
        layout = QHBoxLayout(self)
        for widget in (self.play, self.prev, self.next, self.slider, self.time, self.speed): layout.addWidget(widget)
    ...
    def set_position(self, time_sec: float, duration_sec: float, frame_index: int) -> None:
        ...
        self.slider.blockSignals(True); self.slider.setValue(int(ratio * 10000)); self.slider.blockSignals(False)
        self.time.setText(f"{_format(time_sec)} / {_format(duration_sec)} | frame {frame_index}")


def _format(sec: float) -> str:
    minutes = int(sec // 60); seconds = sec - minutes * 60
