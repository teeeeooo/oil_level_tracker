...
from .foam_temporal_gate import (
    FoamStaticMatch,
...
class CurrentFrameEvidence:
    bundle: MaskBundle
    pre: PreprocessResult
    static_map: np.ndarray | None
    oil_result: OilCanonicalOutcome
    oil_projection: Any
    selected: Any
    selected_calibrated_artifact: bool
    foam: FoamDetectionResult
    foam_layer: Any
    foam_motion: Any
    material_motion_features: dict[str, Any]
    oil_motion: Any
    candidate_assembly: Any
    static_foam_map: np.ndarray | None
    static_foam_match: FoamStaticMatch
    foam_temporal: FoamTemporalDecision
    foam_candidate: Any
    foam_calibrated_artifact: bool


@dataclass(frozen=True, slots=True)
class CurrentFrameProjection:
    detection: PhaseDetection
    debug_profiles: dict[str, list[float]]
    oil_detail: dict[str, Any] | None


class CurrentFrameEvidenceOwner:
    """Acquire independent Oil, Foam, motion and proposal evidence once."""

    def __init__(
        self,
        *,
        evaluate_oil: Callable[
            [str, PreprocessResult, MaskBundle, np.ndarray | None],
            OilCanonicalOutcome,
        ],
        foam_gate: FoamTemporalGate,
        foam_motion_tracker: RegisteredFoamMotionTracker,
        oil_motion_tracker: RegisteredOilMotionTracker,
        static_maps: Mapping[str, np.ndarray],
        static_foam_maps: Mapping[str, np.ndarray],
    ) -> None:
        self.evaluate_oil = evaluate_oil
        self.foam_gate = foam_gate
        self.foam_motion_tracker = foam_motion_tracker
        self.oil_motion_tracker = oil_motion_tracker
        self.static_maps = static_maps
        self.static_foam_maps = static_foam_maps
    ...
    def observe(
        ...
        foam_motion = self.foam_motion_tracker.evaluate(
            glass.id,
            pre.gray,
            bundle.effective_mask,
            foam.mask,
        ...
        oil_motion = self.oil_motion_tracker.evaluate(
            glass.id,
            pre.gray,
            bundle.effective_mask,
        )
        ...
        foam_temporal = self.foam_gate.evaluate(
            glass.id,
            foam,
            settings,
        ...
        return CurrentFrameEvidence(
            bundle=bundle,
            pre=pre,
            static_map=static_map,
            oil_result=oil_result,
            oil_projection=oil_projection,
            selected=selected,
            selected_calibrated_artifact=selected_calibrated_artifact,
            foam=foam,
            foam_layer=foam_layer,
            foam_motion=foam_motion,
            material_motion_features=material_motion_features,
            oil_motion=oil_motion,
            candidate_assembly=candidate_assembly,
            static_foam_map=static_foam_map,
            static_foam_match=static_foam_match,
            foam_temporal=foam_temporal,
            foam_candidate=foam_candidate,
            foam_calibrated_artifact=foam_calibrated_artifact,
        )


class CurrentFrameResultProjector:
    """Project one evidence bundle through compatibility state and output fields."""
    ...
    def project(
        ...
        *,
        ...
        pre = evidence.pre
        oil_result = evidence.oil_result
        oil_projection = evidence.oil_projection
        selected = evidence.selected
        selected_calibrated_artifact = evidence.selected_calibrated_artifact
        foam = evidence.foam
        foam_layer = evidence.foam_layer
        foam_motion = evidence.foam_motion
        material_motion_features = evidence.material_motion_features
        candidate_assembly = evidence.candidate_assembly
        static_foam_map = evidence.static_foam_map
        static_foam_match = evidence.static_foam_match
        foam_temporal = evidence.foam_temporal
        foam_candidate = evidence.foam_candidate
        foam_calibrated_artifact = evidence.foam_calibrated_artifact
        ...
        flags.extend(foam_flags(foam, foam_temporal))
        ...
        if raw_foam_candidate is not None:
            ...
            foam_features["temporal_pending_count"] = float(
                foam_temporal.pending_count
            )
            foam_features["temporal_required_count"] = float(
                foam_temporal.required_count
            )
            foam_features["temporal_front_delta"] = float(
                foam_temporal.front_delta or 0.0
            )
        ...
        debug_metrics = {
            ...
            "foam_bottom_connected_area_ratio": float(
                foam.bottom_connected_area_ratio
            ),
            ...
            "foam_static_artifact_overlap": float(
                static_foam_match.exact_overlap
            ),
            ...
            "foam_static_artifact_pixel_count": (
                0
                if static_foam_map is None
                else int(np.count_nonzero(static_foam_map))
            ),
            ...
            "foam_front_delta": (
                None
                if foam_temporal.front_delta is None
                else float(foam_temporal.front_delta)
            ),
            "foam_adjacent_evidence_available": foam_motion.available,
            "foam_adjacent_exact_overlap": float(foam_motion.exact_overlap),
            "foam_adjacent_tolerant_overlap": float(
                foam_motion.tolerant_overlap
            ),
            "foam_adjacent_reciprocal_overlap": float(
                foam_motion.reciprocal_overlap
            ),
            "foam_adjacent_mask_turnover": float(foam_motion.mask_turnover),
            "foam_adjacent_area_change_ratio": float(
                foam_motion.area_change_ratio
            ),
            "foam_adjacent_front_delta_ratio": float(
                foam_motion.front_delta_ratio
            ),
            ...
            "oil_registered_motion_available": evidence.oil_motion.available,
            "oil_registered_dx": float(evidence.oil_motion.registration_dx),
            "oil_registered_dy": float(evidence.oil_motion.registration_dy),
            "oil_registered_response": float(
                evidence.oil_motion.registration_response
            ),
            "oil_registered_exposure_gain": float(
                evidence.oil_motion.exposure_gain
            ),
            "oil_registered_exposure_offset": float(
                evidence.oil_motion.exposure_offset
            ),
            "foam_min_evidence_score": float(settings.foam_min_evidence_score),
            "foam_strong_evidence_score": float(
                settings.foam_strong_evidence_score
            ),
...
class PhaseDebugProjector:
    """Project debug-only rasters and rows without affecting detection policy."""

    def __init__(self, static_foam_maps: Mapping[str, np.ndarray]) -> None:
        self.static_foam_maps = static_foam_maps
    ...
    def project(
        ...
        glass: GlassInspectionConfig,
        evidence: CurrentFrameEvidence,
        projection: CurrentFrameProjection,
    ) -> PhaseDetectionDebugArtifacts:
        bundle = evidence.bundle
        pre = evidence.pre
        foam = evidence.foam
        foam_temporal = evidence.foam_temporal
        detection = projection.detection
        overlay = frame.copy()
        ellipse = glass.geometry.ellipse
        cv2.ellipse(
            overlay,
            (int(ellipse.center_x), int(ellipse.center_y)),
            (int(ellipse.radius_x), int(ellipse.radius_y)),
            0,
            0,
            360,
            (0, 220, 255),
        ...
        if glass.geometry.zero_line_y is not None:
            extent = ellipse.horizontal_extent_at(glass.geometry.zero_line_y)
            if extent:
                cv2.line(
                    overlay,
        ...
        accepted_mask = (
            foam.mask if foam_temporal.accepted else np.zeros_like(foam.mask)
        )
        static_foam_map = self.static_foam_maps.get(glass.id)
        images = {
            "overlay": overlay,
            ...
            "foam_edge_density": unit_image(foam.edge_density_map),
            "foam_whiteness": unit_image(foam.whiteness_map),
            "foam_texture_evidence": unit_image(foam.texture_evidence_map),
            "foam_glare_excluded_mask": foam.glare_excluded_mask,
            "foam_combined_evidence": unit_image(foam.combined_evidence_map),
            "foam_accepted_component": accepted_mask,
            "static_artifact_map": (
                evidence.static_map
                if evidence.static_map is not None
                else np.zeros_like(bundle.effective_mask)
            ),
            "static_foam_artifact_map": (
                static_foam_map
                if static_foam_map is not None
                else np.zeros_like(bundle.effective_mask)
            ),
        }
        rows = []
        for rank, candidate in enumerate(
            sorted(
                ...
                key=lambda item: (
                    -float(
                        item.features.get("observation_score", item.final_score)
                    ),
                    float(item.y),
                    item.source,
                ),
...
def effective_photometric_metrics(
    pre: PreprocessResult,
...
def unit_image(values: np.ndarray) -> np.ndarray:
...
def overall_confidence(
    ...
    if state is FillState.FULL_WITH_FOAM:
        return max(0.0, min(1.0, visibility * 0.4 + foam * 0.6))
    if state is FillState.UNKNOWN_REVIEW:
        return min(0.35, visibility * 0.35)
    return max(
        0.0,
        min(1.0, visibility * 0.25 + oil * 0.60 + foam * 0.15),
    )
