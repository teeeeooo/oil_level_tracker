...
class ReviewQueryModel:
    ...
    def __init__(self, bundle: ReviewBundle) -> None:
        ...
        self._events = {
            glass_id: tuple(sorted(values, key=lambda item: (item.start_time_sec, item.end_time_sec or item.start_time_sec, item.event_type.value, item.input_order)))
    ...
    def overlay_at(self, glass_id: str, timestamp_sec: float) -> ReviewOverlayData:
        ...
        reasons = review_reasons(sample, glass.detector_settings.minimum_final_confidence) if sample is not None and glass is not None else ()
        boundary_status = ""
    ...
    def nominal_interval(self, glass_id: str) -> float:
        ...
        deltas = [right.timestamp_sec - left.timestamp_sec for left, right in zip(values, values[1:]) if right.timestamp_sec > left.timestamp_sec]
    ...
    def _build_low_confidence_intervals(self, glass_id: str) -> tuple[LowConfidenceInterval, ...]:
        ...
        if glass is None:
            return ()
        flagged = [
            (sample, review_reasons(sample, glass.detector_settings.minimum_final_confidence))
            for sample in self.samples_for_glass(glass_id)
        ]
        flagged = [(sample, reasons) for sample, reasons in flagged if reasons]
        if not flagged:
            return ()
        nominal = self.nominal_interval(glass_id)
        maximum_gap = max(nominal * 1.75, 1e-6)
        groups: list[list[tuple[ReviewTrackingSample, tuple[str, ...]]]] = [[flagged[0]]]
        ...
        intervals = []
        for group in groups:
            representative = min(group, key=lambda item: (item[0].overall_confidence, item[0].timestamp_sec, item[0].frame_index, item[0].input_order))[0]
            reasons = tuple(sorted({reason for _sample, sample_reasons in group for reason in sample_reasons}))
            intervals.append(
                LowConfidenceInterval(
                    glass_id=glass_id,
                    ...
                    minimum_confidence=min(sample.overall_confidence for sample, _reasons in group),
                    reasons=reasons,
...
def review_reasons(sample: ReviewTrackingSample, minimum_confidence: float) -> tuple[str, ...]:
    ...
    for flag in sorted(normalized_flags):
        if flag in _EXPLICIT_REVIEW_FLAGS or "REVIEW" in flag or "FOAM" in flag or "GLARE" in flag or "LOST" in flag:
            reasons.append(flag)
