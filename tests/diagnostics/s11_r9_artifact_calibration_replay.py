...
"""R9 replay after one user-reviewed detector artifact selection."""
...
def run(
    *,
    ...
    payload = run_calibration(
        ...
        run_label="S11-R9-CAL",
        run_note="User-reviewed artifact with bounded calibrated recovery",
    )
    sequence = payload["sequence"]
    truth = payload["user_truth"]
    visual = payload["provisional_visual"]
    if int(sequence["numeric_count"]) != 81:
        raise AssertionError("R9 calibrated sample4 coverage changed")
    if int(sequence["foam_numeric_count"]) != 0:
        raise AssertionError("R9 calibration introduced public Foam")
    if int(truth["numeric_count"]) != 2:
        raise AssertionError("R9 calibration changed checked truth coverage")
    if float(truth["mean_absolute_error_px"]) > 0.5:
        raise AssertionError("R9 calibration worsened checked truth MAE")
    if float(truth["maximum_absolute_error_px"]) > 0.5:
        raise AssertionError("R9 calibration worsened checked truth maximum error")
    if int(visual["visible_range_match_count"]) != 7:
        raise AssertionError("R9 calibration changed reviewed visual matches")
    payload["schema"] = "s11-r9-artifact-calibration-replay-v1"
    payload["uncalibrated_r9_numeric_reference"] = 76
    payload["calibrated_recovery_boundary"] = (
        "High-recall proposals cannot corroborate or replace an already "
        "qualified path; dynamic bootstrap is available only when no qualified "
        "anchor path exists."
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("sample/output/s11-r9-artifact-calibration"),
    )
