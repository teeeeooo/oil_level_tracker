...
"""Deterministic local-video replay for the S11-R10 observation contract."""
...
_MODULE = "tests.diagnostics.s11_r10_calibrated_path_and_layer_replay"
R10_TRACKING_FINGERPRINTS = {
    ...
    # Numeric/state values are unchanged. The five accepted Foam rows now add
    # explicit R10_FOAM_LAYER_SEPARATED provenance.
    "sample3": "79f0b7de6809118a6c8ba65df8ca79838c3babe42e90ad8de4a1d1f15006cc49",
...
def _run_worker(
    *,
    ...
    return replay.run_replay(
        ...
        expected_tracking_fingerprints=R10_TRACKING_FINGERPRINTS,
        run_label="S11-R10",
        run_note="Sparse calibrated Oil path and independent Foam layer replay",
        manifest_schema="s11-r10-calibrated-path-and-layer-worker-replay-v1",
    )
...
def run_r10_replay(
    *,
    ...
    output_root = (
        root / "sample" / "output" / "s11-r10-calibrated-path-and-layer"
    ...
    manifest = {
        "schema": "s11-r10-calibrated-path-and-layer-isolated-replay-v1",
        ...
        "r10_visual_audit": audits,
        "r10_contract": contract,
        "secure_windows_status": "PENDING_R10_PRIVATE_VIDEO_REPLAY",
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("sample/output/s11-r10-calibrated-path-and-layer"),
    )
